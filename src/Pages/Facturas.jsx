...
import { FileText, Plus, Check, Pencil, Trash2 } from "lucide-react";
...
export default function Facturas() {
  ...
  const guardarFactura = async (e) => {
    ...
    try {
      const datos = {
        detalle: nuevaFactura.detalle,

        numeroFactura: nuevaFactura.numeroFactura,

        local: nuevaFactura.local,

        importe: Number(nuevaFactura.importe),
      };

      if (modoEdicion) {
        await updateDoc(doc(db, "facturas", idEditar), datos);
      } else {
  ...
  const confirmarEliminar = async () => {
    try {
      await deleteDoc(doc(db, "facturas", facturaEliminar));
  ...
  return (
    <div className="min-h-screen bg-gray-100 pb-28 ">
      ...
      <div className="flex justify-between  bg-indigo-500 rounded-b-3xl  items-center mb-6 p-4">
        <div>
          <h1 className="text-4xl text-white font-bold">Facturas</h1>

          <p className="text-white">Gestión de facturas</p>
        </div>
        <button
      ...
      <h2 className="text-2xl font-bold m-4">Pendientes de pago</h2>
          ...
          pendientes.map((factura) => (
            ...
            >
              <div className="flex justify-between">
                <div>
                  <p className="text-xs text-gray-400">N° Factura</p>

                  <h3 className="font-bold text-lg">{factura.numeroFactura}</h3>
                </div>
              ...
              <div className="mt-4 space-y-3">
                ...
                <div>
                  ...
                  <p className="font-medium">{factura.local}</p>
                </div>
    ...
    "
      ...
      <h2 className="text-2xl font-bold m-4">Facturas pagadas</h2>

      <div className="space-y-4 m-4">
        {pagadas.length === 0 ? (
          <div className="bg-white p-6 rounded-3xl text-center">
            <p className="text-gray-500">No hay facturas pagadas</p>
          </div>
        ) : (
          pagadas.map((factura) => (
            <div
            ...
            >
              ...
              <div className="mt-4">
                ...
                <p className="text-gray-500">{factura.local}</p>
      ...
      {mostrarEliminar && (
        <div className="fixed inset-0 bg-black/40 flex justify-center items-center z-50 px-4">
          <div className="bg-white w-full max-w-sm rounded-3xl p-6 text-center shadow-2xl">
            ...
            <h2 className="text-2xl font-bold mt-5">Eliminar factura</h2>
